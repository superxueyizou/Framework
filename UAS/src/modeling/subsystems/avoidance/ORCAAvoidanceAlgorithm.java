...
import edu.unc.cs.gamma.rvo2.RVO2;
import edu.unc.cs.gamma.rvo2.RVOSimulator;
import edu.unc.cs.gamma.rvo2.Vector2;
...
public class ORCAAvoidanceAlgorithm extends AvoidanceAlgorithm{
	...
	public void initRVO2Simulator()
	{
		...
		for(int i=0; i<state.uasBag.size(); i++)
		{
			...
			rvoSimulator.addAgent(location);			
		}
...
//				double velY = agent.getSpeed()*Math.sin(Math.toRadians(agent.getBearing()))+state.random.nextGaussian();
	...
	public void setPreferredVelocity(COModel state) 
	{
		if (!isGoalReached) 
		{
			if (RVO2.absSq(targetPosInRVOSimulator.sub(rvoSimulator.getAgentPosition(hostUASIDInRVOSimulator))) < 1) 
			{	
